...
버전 전환은 delta를 쓸 수 없어 언제나 전체 패키지를 받습니다. delta는 직전 버전에서 최신으로 가는 한 방향으로만 존재하고, Velopack도 다운그레이드에는 full만 허용하기 때문입니다. 그만큼 오래 걸리는 작업이라 진행률을 화면에 표시합니다. 다운로드가 끝나도 프로세스를 강제로 끊지 않고, CefSharp가 쿠키와 IndexedDB 파일을 놓도록 정상 종료한 뒤 패키지를 적용합니다.
